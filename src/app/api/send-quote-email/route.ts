...
export async function POST(request: NextRequest) {
  if (!RESEND_API_KEY || !RESEND_FROM_EMAIL) {
    console.log('[send-quote-email] Resend not configured. Skipping email send.');
  ...
  if (!ref || !customerEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customerEmail)) {
    return NextResponse.json({ error: 'Invalid submission data' }, { status: 400 });
  }

  // Duplicate send guard
  if (sentRefs.has(ref)) {
    console.log(`[send-quote-email] Duplicate send blocked for ref: ${ref}`);
  ...
  const safeServices = Array.isArray(selectedServices) ? selectedServices.map(escapeHtml).join(', ') : '';
  const safeDesc = escapeHtml(String(projectDescription || ''));
  const safeType = escapeHtml(String(projectType || ''));
  const safeBudget = escapeHtml(String(budget || ''));
  const safeCity = escapeHtml(String(city || ''));
  const safePhone = escapeHtml(String(phone || ''));
  const safeIndustry = escapeHtml(String(industry || ''));
  const safeDate = escapeHtml(String(preferredStartDate || 'Not specified'));
  const safeRefLink = escapeHtml(String(referenceLink || 'Not provided'));
  const safeUtmSource = escapeHtml(String(utmSource || 'Direct'));
  const safeUtmMedium = escapeHtml(String(utmMedium || 'None'));
  const safeUtmCampaign = escapeHtml(String(utmCampaign || 'None'));
...
<body style="font-family: 'Segoe UI', Arial, sans-serif; background: #f8fafc; margin: 0; padding: 0;">
  <div style="max-width: 600px; margin: 40px auto; background: #fff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.08);">
    <div style="background: linear-gradient(135deg, #0758F9, #00CF75); padding: 32px 40px; text-align: center;">
      <h1 style="color: #fff; margin: 0; font-size: 24px; font-weight: 800; letter-spacing: -0.5px;">DIGITALNYNE</h1>
      <p style="color: rgba(255,255,255,0.85); margin: 8px 0 0; font-size: 14px;">Growth Studio</p>
    </div>
    <div style="padding: 40px;">
      <h2 style="color: #0f172a; font-size: 20px; font-weight: 700; margin: 0 0 8px;">We have received your enquiry!</h2>
      <p style="color: #64748b; font-size: 15px; line-height: 1.6; margin: 0 0 24px;">
        Hi ${safeName}, thank you for reaching out to DIGITALNYNE Growth Studio. We have received your quote request and our team will review it and get back to you shortly.
      </p>
      <div style="background: #f1f5f9; border-radius: 12px; padding: 20px; margin-bottom: 24px; text-align: center;">
        ...
        <p style="color: #0758F9; font-size: 28px; font-weight: 800; margin: 0;">${safeRef}</p>
        <p style="color: #94a3b8; font-size: 12px; margin: 8px 0 0;">Please save this reference for future communication.</p>
      </div>
      <p style="color: #64748b; font-size: 14px; line-height: 1.6; margin: 0 0 24px;">
        If you have any questions or would like to add more details, please reply to this email or contact us directly.
      </p>
...
<body style="font-family: 'Segoe UI', Arial, sans-serif; background: #f8fafc; margin: 0; padding: 0;">
  <div style="max-width: 600px; margin: 40px auto; background: #fff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.08);">
    <div style="background: #0f172a; padding: 24px 32px;">
      <h1 style="color: #fff; margin: 0; font-size: 18px; font-weight: 700;">New Quote Request</h1>
      <p style="color: rgba(255,255,255,0.5); margin: 4px 0 0; font-size: 13px;">Reference: ${safeRef}</p>
    </div>
    <div style="padding: 32px;">
      <table style="width: 100%; border-collapse: collapse;">
        <tr><td style="padding: 8px 0; color: #64748b; font-size: 13px; width: 40%; vertical-align: top;">Submission Type</td><td style="padding: 8px 0; color: #0f172a; font-size: 13px; font-weight: 600;">Quote Request</td></tr>
        ...
        <tr><td style="padding: 8px 0; color: #64748b; font-size: 13px; vertical-align: top;">Business</td><td style="padding: 8px 0; color: #0f172a; font-size: 13px; font-weight: 600;">${safeBusiness}</td></tr>
        <tr><td style="padding: 8px 0; color: #64748b; font-size: 13px; vertical-align: top;">Industry</td><td style="padding: 8px 0; color: #0f172a; font-size: 13px; font-weight: 600;">${safeIndustry}</td></tr>
        <tr><td style="padding: 8px 0; color: #64748b; font-size: 13px; vertical-align: top;">City</td><td style="padding: 8px 0; color: #0f172a; font-size: 13px; font-weight: 600;">${safeCity}</td></tr>
        <tr><td colspan="2" style="padding: 4px 0;"><hr style="border: none; border-top: 1px solid #e2e8f0;"></td></tr>
        <tr><td style="padding: 8px 0; color: #64748b; font-size: 13px; vertical-align: top;">Services Requested</td><td style="padding: 8px 0; color: #0f172a; font-size: 13px; font-weight: 600;">${safeServices}</td></tr>
        <tr><td style="padding: 8px 0; color: #64748b; font-size: 13px; vertical-align: top;">Project Type</td><td style="padding: 8px 0; color: #0f172a; font-size: 13px; font-weight: 600;">${safeType}</td></tr>
        <tr><td style="padding: 8px 0; color: #64748b; font-size: 13px; vertical-align: top;">Budget</td><td style="padding: 8px 0; color: #0f172a; font-size: 13px; font-weight: 600;">${safeBudget}</td></tr>
        <tr><td style="padding: 8px 0; color: #64748b; font-size: 13px; vertical-align: top;">Preferred Start</td><td style="padding: 8px 0; color: #0f172a; font-size: 13px; font-weight: 600;">${safeDate}</td></tr>
        <tr><td style="padding: 8px 0; color: #64748b; font-size: 13px; vertical-align: top;">Reference Link</td><td style="padding: 8px 0; color: #0f172a; font-size: 13px; font-weight: 600;">${safeRefLink}</td></tr>
        <tr><td colspan="2" style="padding: 4px 0;"><hr style="border: none; border-top: 1px solid #e2e8f0;"></td></tr>
      ...
      <div style="background: #f1f5f9; border-radius: 8px; padding: 16px; margin-top: 16px;">
        <p style="color: #64748b; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin: 0 0 8px;">Project Description</p>
        <p style="color: #0f172a; font-size: 13px; line-height: 1.6; margin: 0;">${safeDesc}</p>
      </div>
    </div>
  </div>
</body>
  ...
  try {
    const [customerRes, adminRes] = await Promise.allSettled([
      fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          subject: `Enquiry Received: ${safeRef} | DIGITALNYNE Growth Studio`,
          html: customerHtml,
        }),
      }),
      fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          subject: `New Quote Request: ${safeRef} from ${safeName} (${safeBusiness})`,
          html: adminHtml,
        }),
      }),
    ]);
    ...
    if (!customerOk) {
      ...
      console.error('[send-quote-email] Customer email failed:', reason);
    }
    if (!adminOk) {
      const reason = adminRes.status === 'rejected' ? adminRes.reason : await adminRes.value.text().catch(() => 'unknown');
      console.error('[send-quote-email] Admin email failed:', reason);
    }

    return NextResponse.json({ success: true, emailSent: customerOk || adminOk });
  } catch (err) {
    console.error('[send-quote-email] Unexpected error:', err instanceof Error ? err.message : 'unknown');
    return NextResponse.json({ success: true, emailSent: false });
  }
}
